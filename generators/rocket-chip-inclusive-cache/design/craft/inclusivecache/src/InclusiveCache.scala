...
import freechips.rocketchip.subsystem.{BankedL2Key} // import freechips.rocketchip.subsystem.{SubsystemBankedCoherenceKey}
    ...
    extends LazyModule
{
  ...
  val ctrls = control.map { c =>
    val nCtrls = if (c.bankedControl) p(BankedL2Key/*SubsystemBankedCoherenceKey*/).nBanks else 1
