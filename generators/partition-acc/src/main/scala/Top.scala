...
class PartitionAccelImp(outer: PartitionAccel)(implicit p: Parameters) 
  ...
  // 为cmd_router的未连接端口赋默认值
  cmd_router.io.partition_busy := false.B
  cmd_router.io.partition_finished := false.B
  ...
  // cache_line_loader.io.consumer <> DontCare
  ...
  // tuple_length_loader.io.consumer <> DontCare
  ...
  // key_info_loader.io.consumer <> DontCare
  ...
  // cache_line_writer.io.decompress_dest_info <> DontCare
  // cache_line_writer.io.memwrites_in <> DontCare
