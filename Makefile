...
# function print_log 
# print_log = echo -e "\033[1;34m[Make state]\033[0m $(1)"
BLUE=\033[1;34m
NC=\033[0m
define print_log
	@printf "$(BLUE)[Make state] %s $(NC)\n" "$(1)"
endef
...
SBT := java $(SBT_BUILD_ARGS) $(JAVA_OPTIONS) $(SBT_EXTRA_ARGS) -Dsbt.io.virtual=false -Dsbt.server.autostart=false -jar $(realpath sbt-launch.jar)
...
workspace/$(CONFIG)/system-$(BOARD)/RocketSystem.fir: workspace/$(CONFIG)/system.dts $(wildcard bootrom/*) workspace/gcc/riscv
	@$(call print_log,rocket-assembly)
...
workspace/$(CONFIG)/system-$(BOARD).sv: workspace/$(CONFIG)/system-$(BOARD)/RocketSystem.fir
	...
	$(FIRRTL) -i $< -o RocketSystem.v --compiler verilog \
	  ...
	  --target:fpga
	cp workspace/$(CONFIG)/system-$(BOARD)/RocketSystem.v workspace/$(CONFIG)/system-$(BOARD).sv
	# Copy any additional Verilog files generated by FIRRTL (e.g., for RoCC accelerators)
	for vfile in workspace/$(CONFIG)/system-$(BOARD)/*.v; do \
		if [ "$$vfile" != "workspace/$(CONFIG)/system-$(BOARD)/RocketSystem.v" ] && [ -f "$$vfile" ]; then \
...
# If any vivado process was freezed, open vivado gui and do 'Create New Runs'.
# may be try PRJ_NUM=2, 3, 4, etc?
PRJ_NUM	?= 1
...
synthesis   = $(proj_path)/$(proj_name).runs/synth_$(PRJ_NUM)/riscv_wrapper.dcp
...
# 	fi
	...
	echo "set additional_verilog_files {}" >>$@
	for vfile in workspace/$(CONFIG)/*.sv; do \
		if [ "$$vfile" != "workspace/$(CONFIG)/system-$(BOARD).sv" ] && [ -f "$$vfile" ]; then \
			echo "lappend additional_verilog_files [file normalize \"$$(realpath $$vfile)\"]" >>$@; \
		fi \
	done
